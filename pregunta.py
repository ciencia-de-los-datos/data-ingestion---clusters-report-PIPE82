...
def ingest_data():
    ...
    #df=pd.read_fwf('C:/Users/Andres/Documents/GitHub/data-ingestion---clusters-report-PIPE82/clusters_report.txt',widths=[9, 16, 16, 77])
    ...
    #print(df.columns)
    df=df.ffill()
    df=df.groupby(df.columns[0]).agg({df.columns[1]:'first',df.columns[2]:'first',df.columns[3]: lambda x: ' '.join(x)})
    df = df.reset_index()
